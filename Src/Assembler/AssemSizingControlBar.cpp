/////////////////////////////////////////////////////////////////////////
//
// CAssemSizingControlBar            Version 2.43
//
...
#include "stdafx.h"
#include "AssemSizingControlBar.h"
...
/////////////////////////////////////////////////////////////////////////
// CAssemSizingControlBar

IMPLEMENT_DYNAMIC(CAssemSizingControlBar, baseCSizingControlBar);

CAssemSizingControlBar::CAssemSizingControlBar()
{
...
BEGIN_MESSAGE_MAP(CAssemSizingControlBar, baseCSizingControlBar)
    //{{AFX_MSG_MAP(CAssemSizingControlBar)
    ON_WM_CREATE()
    ON_WM_PAINT()
    ON_WM_NCPAINT()
    ON_WM_NCCALCSIZE()
...
BOOL CAssemSizingControlBar::Create(LPCTSTR lpszWindowName, CWnd* pParentWnd,
...
BOOL CAssemSizingControlBar::Create(LPCTSTR lpszWindowName,
...
/////////////////////////////////////////////////////////////////////////
// CAssemSizingControlBar operations
#if defined(_SCB_REPLACE_MINIFRAME) && !defined(_SCB_MINIFRAME_CAPTION)
void CAssemSizingControlBar::EnableDocking(DWORD dwDockStyle)
{
...
/////////////////////////////////////////////////////////////////////////
// CAssemSizingControlBar message handlers

int CAssemSizingControlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
LRESULT CAssemSizingControlBar::OnSetText(WPARAM wParam, LPARAM lParam)
{
...
const BOOL CAssemSizingControlBar::IsFloating() const
{
    return !IsHorzDocked() && !IsVertDocked();
}

const BOOL CAssemSizingControlBar::IsHorzDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

const BOOL CAssemSizingControlBar::IsVertDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_LEFT ||
        m_nDockBarID == AFX_IDW_DOCKBAR_RIGHT);
}

const BOOL CAssemSizingControlBar::IsSideTracking() const
{
...
CSize CAssemSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    CAssemSCBDockBar* pDockBar = (CAssemSCBDockBar*) m_pDockBar;
...
CSize CAssemSizingControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void CAssemSizingControlBar::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
...
// Mouse Handling
//
void CAssemSizingControlBar::OnLButtonDown(UINT nFlags, CPoint point)
{
    if (m_pDockBar != NULL)
    {
        // start the drag
...
void CAssemSizingControlBar::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    if (m_pDockBar != NULL)
    {
        // toggle docking
...
void CAssemSizingControlBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
    UNUSED_ALWAYS(point);
...
void CAssemSizingControlBar::OnLButtonUp(UINT nFlags, CPoint point)
{
    if (m_bTracking)
        StopTracking();
...
void CAssemSizingControlBar::OnRButtonDown(UINT nFlags, CPoint point)
{
    if (m_bTracking)
        StopTracking();
...
void CAssemSizingControlBar::OnMouseMove(UINT nFlags, CPoint point)
{
    if (m_bTracking)
    {
...
void CAssemSizingControlBar::OnCaptureChanged(CWnd *pWnd)
{
...
void CAssemSizingControlBar::OnNcCalcSize(BOOL bCalcValidRects,
...
void CAssemSizingControlBar::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
    CRect rc(pRc);
...
void CAssemSizingControlBar::OnNcPaint()
{
    ...
    mdc.FillRect(rcDraw, CBrush::FromHandle(
		(HBRUSH) GetClassLong(m_hWnd, -8))); //오류	2	error C2065: 'GCL_HBRBACKGROUND' : undeclared identifier	d:\Documents and Settings\Administrator\바탕 화면\KaimoPre_GUI_ASSEM\Src\Assembler\AssemSizingControlBar.cpp	536	
...
void CAssemSizingControlBar::NcPaintGripper(CDC* pDC, CRect rcClient)
{
    UNUSED_ALWAYS(pDC);
    UNUSED_ALWAYS(rcClient);
}

void CAssemSizingControlBar::OnPaint()
{
    // overridden to skip border painting based on clientrect
    CPaintDC dc(this);
}

LRESULT CAssemSizingControlBar::OnNcHitTest(CPoint point)
{
    CRect rcBar, rcEdge;
    GetWindowRect(rcBar);
...
void CAssemSizingControlBar::OnSettingChange(UINT uFlags, LPCTSTR lpszSection)
{
...
void CAssemSizingControlBar::OnSize(UINT nType, int cx, int cy)
{
    UNUSED_ALWAYS(nType);
...
void CAssemSizingControlBar::OnClose()
{
...
/////////////////////////////////////////////////////////////////////////
// CAssemSizingControlBar implementation helpers

void CAssemSizingControlBar::StartTracking(UINT nHitTest, CPoint point)
{
    SetCapture();
    ...
    else
    {
        ...
        for (int i = 0; i < arrSCBars.GetSize(); i++)
        {
            CAssemSizingControlBar* pBar = arrSCBars[i];
...
void CAssemSizingControlBar::StopTracking()
{
...
void CAssemSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ASSERT(!IsFloating());
    ...
    else
    {
        ...
        // nGrowing is growing
        nDelta = abs(nDelta);
        CAssemSizingControlBar* pBar = arrSCBars[nGrowingBar];
...
void CAssemSizingControlBar::OnTrackInvertTracker()
{
    ASSERT(m_bTracking);
...
BOOL CAssemSizingControlBar::GetEdgeRect(CRect rcWnd, UINT nHitTest,
                                    CRect& rcEdge)
{
    rcEdge = rcWnd;
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
...
UINT CAssemSizingControlBar::GetEdgeHTCode(int nEdge)
{
...
void CAssemSizingControlBar::GetRowInfo(int& nFirst, int& nLast, int& nThis)
{
...
void CAssemSizingControlBar::GetRowSizingBars(CAssemSCBArray& arrSCBars)
{
    int nThis; // dummy
    GetRowSizingBars(arrSCBars, nThis);
}

void CAssemSizingControlBar::GetRowSizingBars(CAssemSCBArray& arrSCBars, int& nThis)
{
    arrSCBars.RemoveAll();
    ...
    {
        CAssemSizingControlBar* pBar =
            (CAssemSizingControlBar*) m_pDockBar->m_arrBars[i];
        if (HIWORD(pBar) == 0) continue; // placeholder
        if (!pBar->IsVisible()) continue;
        if (pBar->IsKindOf(RUNTIME_CLASS(CAssemSizingControlBar)))
        {
            if (pBar == this)
...
BOOL CAssemSizingControlBar::NegotiateSpace(int nLengthTotal, BOOL bHorz)
{
    ...
    {
        pBar = (CAssemSizingControlBar*) m_pDockBar->m_arrBars[i];
        if (HIWORD(pBar) == 0) continue; // placeholder
        if (!pBar->IsVisible()) continue;
        BOOL bIsSizingBar = 
            pBar->IsKindOf(RUNTIME_CLASS(CAssemSizingControlBar));
...
void CAssemSizingControlBar::AlignControlBars()
{
    int nFirst, nLast, nThis;
    GetRowInfo(nFirst, nLast, nThis);
...
void CAssemSizingControlBar::OnUpdateCmdUI(CFrameWnd* pTarget,
...
void CAssemSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ASSERT_VALID(this);
...
void CAssemSizingControlBar::SaveState(LPCTSTR lpszProfileName)
{
...
void CAssemSizingControlBar::GlobalLoadState(CFrameWnd* pFrame,
                                        LPCTSTR lpszProfileName)
{
    ...
    {
        CAssemSizingControlBar* pBar = 
            (CAssemSizingControlBar*) pFrame->m_listControlBars.GetNext(pos);
        ASSERT(pBar != NULL);
        if (pBar->IsKindOf(RUNTIME_CLASS(CAssemSizingControlBar)))
...
void CAssemSizingControlBar::GlobalSaveState(CFrameWnd* pFrame,
                                        LPCTSTR lpszProfileName)
{
    POSITION pos = pFrame->m_listControlBars.GetHeadPosition();
    while (pos != NULL)
...
CAssemSizingControlBar* CSCBMiniDockFrameWnd::GetSizingControlBar()
{
    ...
    if (!pWnd->IsKindOf(RUNTIME_CLASS(CAssemSizingControlBar)))
        return NULL;

    return (CAssemSizingControlBar*) pWnd;
}

void CSCBMiniDockFrameWnd::OnSize(UINT nType, int cx, int cy) 
{
    CAssemSizingControlBar* pBar = GetSizingControlBar();
