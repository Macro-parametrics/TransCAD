...
PME_IMPLEMENT_RUNTIME_TYPE(PmeStdAssemblyAngle, PmeObject)

PmeStdAssemblyAngle::PmeStdAssemblyAngle(PmeAssembly * pAssembly,
...
: PmeStdAssemblyConstraint(pAssembly)
{
	...
	m_Angle			= M_PI * ( angle / 180 );
}

PmeStdAssemblyAngle::~PmeStdAssemblyAngle(void)
{
}
...
*/
void PmeStdAssemblyAngle::UpdateConstraint()
{
	...
	if ( *pCoEntity->type_name() == *facetype && *pRefEntity->type_name() == *facetype )
	{
		...
		if ( IsPlanarFace( (FACE *)pCoEntity ) && IsPlanarFace( (FACE *)pRefEntity ) )
		{
			...
			SetAngle( coFaceLCSYS, refFaceLCSYS );
		}
	...
	else if ( *pCoEntity->type_name() == *edgetype && *pRefEntity->type_name() == *facetype )
	{
		...
		SetAngle( refFaceLCSYS, coEdge, 0 );
	}
	...
	else if ( *pCoEntity->type_name() == *facetype && *pRefEntity->type_name() == *edgetype )
	{
		...
		SetAngle( coFaceLCSYS, refEdge, 1 );
	}
}

/**
	@brief					Apply angle constraint between faces.
...
*/
void PmeStdAssemblyAngle::SetAngle( MrAxis2 coFaceLocalCSYS, MrAxis2 refFaceLocalCSYS )
{
...
/**
	@brief			Apply angle constraint between edges.

	@param pCoEdge	Constrained edge.

	@param pRefEdge	Reference edge.
*/
void PmeStdAssemblyAngle::SetAngle( EDGE * pCoEdge, EDGE * pRefEdge )
{
...
/**
	@brief					Apply angle constraint between face and edge vice versa.
...
*/
void PmeStdAssemblyAngle::SetAngle( MrAxis2 faceLocalCSYS, EDGE * pEdge, int flag )
{
	// Vector of straight edge from start to end
...
void PmeStdAssemblyAngle::DoAngle(void)
{
	MrMatrix rMatrix;
	...
	if ( initialAngle > m_Angle )
	{
		rotationAngle = initialAngle - m_Angle;
	}

	else if ( initialAngle < m_Angle )
	{
		rotationAngle = (-1) * ( m_Angle - initialAngle );
	}
